...
async def run_background_scan(state: dict, demo: bool = True):
    ...
    try:
        ...
        for device in devices:
            # open_ports comes back as dict {int: str} from scan_ports — keep it that way
            ports = scanner.scan_ports(device["ip"])
            device["open_ports"] = ports        # dict form for fingerprinter/scorecard
            device["open_ports_dict"] = ports   # alias
